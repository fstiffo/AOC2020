...
let firstHalf () =
    ...
    let binaryPartions =
        ...
        |> Array.map
            (fun l ->
                let rowBinPart =
                    l.[..6].Replace('B', '1').Replace('F', '0')

                let colBinPart =
                    l.[7..].Replace('R', '1').Replace('L', '0')

                System.Convert.ToByte(rowBinPart, 2), System.Convert.ToByte(colBinPart, 2))

    binaryPartions
    |> Array.map
        (fun (rowBinPart, colBinPart) ->
            let row, col =
                rowBinPart &&& byte (127), colBinPart &&& byte (7) // find row and column of the seat
